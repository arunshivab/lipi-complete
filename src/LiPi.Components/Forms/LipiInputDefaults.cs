...
public sealed class LipiInputDefaults
{
    ...
    // (LiPi.Components.CheckboxGroupDensity.Standard) avoids the
    ...
    // ==========================================================================
    // DateTime family defaults (DateTime migration → LiPi.Components.Forms / A54)
    // Configure once in Program.cs; per-component params override when set.
    // A HIS sets these from clinic config (date_format / time_format /
    // week_start_day / timezone_id) so the pickers stay clinic-unaware.
    // ==========================================================================

    /// <summary>Default date display/parse format for the Date/Time picker family.
    /// Design tokens D/DD/M/MM/MMM/MMMM/YY/YYYY with / - . space separators.
    /// Default: "DD/MM/YYYY" (India). Per-component <c>Format</c> overrides.</summary>
    public string DefaultDateFormat { get; set; } = "DD/MM/YYYY";

    /// <summary>Default time display/parse format. "12h" | "24h".
    /// Default: "24h" (clinical-safety — no AM/PM medication-time ambiguity).
    /// Per-component <c>TimeFormat</c> overrides.</summary>
    public string DefaultTimeFormat { get; set; } = "24h";

    /// <summary>Default first day of the week for calendar grids and "this week"/"last week"
    /// math. Default: <see cref="DayOfWeek.Sunday"/> (India + US). European deployments
    /// typically set Monday. Per-component <c>WeekStart</c> overrides.</summary>
    public DayOfWeek DefaultWeekStart { get; set; } = DayOfWeek.Sunday;

    /// <summary>Default time source for Today/Tomorrow/Now and relative presets.
    /// Default: <see cref="LiPi.Components.Forms.LipiTimeSource.Server"/> — the SaaS-safe
    /// choice (trust the NTP-synced server, not a client clock that may be mis-set).
    /// Per-component <c>TimeSource</c> overrides.</summary>
    public LipiTimeSource DefaultTimeSource { get; set; } = LipiTimeSource.Server;

    /// <summary>Default explicit zone used when <c>TimeSource = SpecificZone</c>.
    /// Default: null. A HIS sets this to the clinic zone (e.g.
    /// <see cref="LiPi.Components.Forms.LipiTimeZones.IndiaIST"/>). Per-component
    /// <c>TimeZone</c> overrides.</summary>
    public TimeZoneInfo? DefaultTimeZone { get; set; } = null;
}
